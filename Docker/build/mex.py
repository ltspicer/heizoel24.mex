...
USERNAME = os.getenv("USERNAME") # Deine Email Adresse bei HeizOel24
PASSWORD = os.getenv("PASSWORD") # Dein Passwort bei HeizOel24
MEX_ID = int(os.getenv("MEX_ID","1"))     # Falls mehrere MEX Geräte, hier ID eintragen (2, 3, 4, ...)

BROKER_ADDRESS = os.getenv("BROKER_ADDRESS")  # MQTT Broker IP (da wo der MQTT Broker läuft)
MQTT_USER = os.getenv("MQTT_USER") # MQTT User      (im MQTT Broker definiert)
MQTT_PASS = os.getenv("MQTT_PASS") # MQTT Passwort  (im MQTT Broker definiert)
MQTT_PORT = int(os.getenv("MQTT_PORT","1883")) # MQTT Port      (default: 1883)

#----------------------------- Kann normalerweise belassen werden: -------------------------------#

REFERENCE_MONTH = int(os.getenv("REFERENCE_MONTH", "6"))  # Stichmonat (Monatsende) für Jahresverbrauchs Berechnung (1-12)
                                                   # 0 deaktiviert die Übertragung der Jahresverbrauchs Daten

MQTT_ACTIVE = os.getenv("MQTT_ACTIVE", True)  # Auf False, wenn nichts MQTT published werden soll

LESS_DATA = os.getenv("LESS_DATA", True)      # Weniger Zukunftsdaten abrufen (nur alle 14 Tage)
CREATE_JSON = os.getenv("CREATE_JSON", True)  # True erstellt:
                                #   CalculatedRemaining.json
                                #   OilUsage.json
                                #   Items.json

JSON_PATH = os.getenv("JSON_PATH", "")    # Pfad für die Json Datei. Standardpfad ist bei Script.
                                # sonst zBsp.: JSON_PATH = "/home/pi/"

DELAY = os.getenv("DELAY", False)   # Auf True setzen, wenn der MQTT Broker nur die 1. Zeile empfängt
DEBUG = os.getenv("DEBUG", False)   # True = Debug Infos auf die Konsole.
